...
import {searchAssignments, showCellAssignment, submitCellAssignment} from "../../../actions/boardToolsAction"
        ...
        if (pExists !== -1) {
            if (tasks[pExists].id === newTask.id) {
                ...
                this.setState({
                    value: {
                        ...newValue,
                        assignments: tasks.map(a => a.priority === newTask.priority ? newTask : a)
                    }
                })
            }
        } else if (idExists !== -1) {
            ...
            <Spin spinning={!!loading.states[SUBJECT_BOARD_ASSIGNMENTS]}>
              <Row gutter={[32, 0]}>
                <Col md={8} sm={24} xs={24}>
                  ...
                  >
                    {append.map((task) => this.renderMenuOption(task, selectedTask))}
