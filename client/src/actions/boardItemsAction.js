...
import { FETCH_BOARD_ITEMS, LOADING, LOADING_DONE, NEW_BOARD_ITEMS, REPLACE_BOARD_ITEM } from "./types"
 ...
 * @returns {function(...[*]=)}
 */
export const fetchBoardItems = (identifier, fromDate, toDate) => async (dispatch) => {
  dispatch({ type: LOADING, subject: SUBJECT_BOARD_ITEMS })
  const res = await axios.get(`/api/boards/${identifier}/items`, {
    params: {
 ...
 * @return {function(...[any]=)}
 */
export const copyBoardItems = (identifier, sourceFrom, sourceTo, targetFrom, targetTo, shift) => async (dispatch) => {
  dispatch({ type: LOADING, subject: SUBJECT_BOARD_ITEMS })

  await axios.delete(`/api/boards/${identifier}/items`, {
    params: {
  ...
  const res = await axios.post(`/api/boards/${identifier}/items`, null, {
    params: {
 ...
 * @returns {function(...[*]=)}
 */
export const updateBoardItem = (identifier, id, boardItem) => async (dispatch) => {
  dispatch({ type: LOADING, subject: SUBJECT_BOARD_ITEMS })

  const res = await axios.put(`/api/boards/${identifier}/items/${id}`, boardItem)
    .catch(errorDispatch(dispatch, SUBJECT_BOARD_ITEMS))
...
export const sendBoardItem = (identifier, item) => async (dispatch) => {
  ...
  const res = await axios.post(`/api/boards/${identifier}/items`, item)
    .catch(errorDispatch(dispatch, SUBJECT_BOARD_ITEMS))
