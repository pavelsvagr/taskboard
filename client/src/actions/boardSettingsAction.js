...
export const fetchBoardSettings = (identifier, date) => async (dispatch) => {
  ...
  const res = await axios
    .get(`/api/boards/${identifier}/settings/${date.format("YYYY-MM-DD")}`)
...
export const updateBoardSettings = (
  ...
  if (send) {
    dispatch({ type: LOADING, subject: SUBJECT_BOARD_SETTINGS })
    const res = await axios
      .put(`/api/boards/${identifier}/settings/${dateString}`, values)
      .catch(errorDispatch(dispatch, SUBJECT_BOARD_SETTINGS))
    ...
    dispatch({
      type: FETCH_BOARD_SETTINGS,
      payload: { ...values, date: dateString },
    })
  }
}
