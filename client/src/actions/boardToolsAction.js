...
export const showCellAssignment = (
  memberId,
  teamId,
  priority,
  boardItem = null
...
export const submitCellAssignment = (boardItem, assignment) => (
  dispatch,
  getState
) => {
...
export const pasteBoardRow = (board, date, member, copiedRow, oldRow) => async (
  dispatch
) => {
  if (!oldRow) {
...
export const searchAssignments = (search) => (dispatch) => {
  dispatch({ type: BOARD_TASK_SEARCH, search })
}
 ...
 * @returns {function(...[*]=)}
 */
export const pasteBoardTasks = (board, targetDate, boardCopy) => async (
  dispatch
) => {
  if (!board || !boardCopy) {
    return
...
export const activateBoardMember = (board, date, member, settings) =>
  ...
  if (
    ...
    updateBoardSettings(board.identifier, dateFrom, {
      ...settings,
      deactivated,
    })(dispatch)
  }
